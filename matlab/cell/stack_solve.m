...
%     [rho_air_ca_in,cv_air_ca_in,cp_air_ca_in,k_air_ca_in,mu_air_ca_in]=PropAir(p_air_ca_in,T_ca_in);
    ...
    flow_w_co(i)=flow_water_ca_out;
    flow_w_g(i)=flow_water_gen;
...
%     resistivity_mem=(thick_mem/area_mem)*181.6*(1+0.03*currentDen+0.062*(T_cell/303)^2*currentDen^2.5)/((sigma_an-0.634-3*currentDen)*exp(4.18*(1-303/T_cell)));
    ...
    m_water_ca=m_water_ca+(flow_water_ca_in-flow_water_ca_out+flow_water_gen-flow_water_mem);
    m_H2_an=m_H2_an+(flow_H2_an_in-flow_H2_an_out-flow_H2_react);
    m_N2_an=m_N2_an+(flow_N2_an_in-flow_N2_an_out);
    m_water_an=m_water_an+(flow_water_an_in-flow_water_an_out-flow_water_mem);
    T_ca=T_ca+(h_ca*(T_cell-T_ca)+(cp_air_ca_in*flow_air_ca_in+cp_wv_ca_in*flow_water_ca_in)*T_ca_in-(cp_O2_ca*(flow_O2_ca_out+flow_O2_react)+cp_N2_ca*flow_N2_ca_out+cp_wv_ca*flow_water_ca_out)*T_ca)/(cp_O2_ca*m_O2_ca+cp_N2_ca*m_N2_ca+cp_wv_ca*m_wv_ca+cv_wl_ca*m_wl_ca);
    T_an=T_an+(h_an*(T_cell-T_an)+(cp_H2_an_in*flow_H2_an_in+cp_wv_an_in*flow_water_an_in)*T_an_in-(cp_H2_an*(flow_H2_an_out+flow_H2_react)+cp_N2_an*flow_N2_an_out+cp_wv_an*flow_water_an_out)*T_an)/(cp_H2_an*m_H2_an+cp_N2_an*m_N2_an+cp_wv_an*m_wv_an+cv_wl_an*m_wl_an);
    T_cell=T_cell+(h_ca*(T_ca-T_cell)+h_an*(T_an-T_cell)+h_amb*(T_amb-T_cell)+h_cl*(T_cl-T_cell)+cp_H2_an*flow_H2_react*(T_an-T_cell)+cp_O2_ca*flow_O2_react*(T_ca-T_cell)-cp_wv_ca*flow_water_gen*(T_ca-T_cell)+flow_H2_react*dH_react-num_cell*volt_cell*current)/(Cp_stack*m_cell*num_cell);
...
t=[1:3600];
plot(t,flow_O2_co,'r',t,flow_H2_ao,'g',t,flow_N2_co,'m',t,flow_N2_ao,':m');
figure;
